...
|---------|-----------|---------------------|
| **Documento Electrónico** (`electronicDocument`) | Término genérico que engloba los tres tipos de documentos electrónicos del sistema: factura, nota crédito y nota débito | Documento, archivo, comprobante |
| **Factura DIAN** (`invoice`) | Documento electrónico de tipo factura enviado hacia la DIAN. Su estructura de request es `RootInvoice`. No aplica para documentos Éxito | Factura Éxito, factura genérica |
| **Nota Crédito DIAN** (`creditNote`) | Documento electrónico de tipo nota crédito enviado hacia la DIAN. Usa la misma estructura de request `RootInvoice`. No aplica para documentos Éxito | Nota crédito Éxito, nota crédito genérica |
| **Nota Débito DIAN** (`debitNote`) | Documento electrónico de tipo nota débito enviado hacia la DIAN. Usa la misma estructura de request `RootInvoice`. No aplica para documentos Éxito | Nota débito Éxito, nota débito genérica |
| **Documento Éxito** (`electronicDocumentExito`) | Documento electrónico originado desde sistemas Éxito (POS, ERP) con estructura `RootExitoDto`. Es transformado a estructura DIAN para enviarse al OT Tecnológico. Puede ser rechazado en la capa media Éxito y gestionado desde la clínica para su reenvío | Documento DIAN, documento genérico |
| **Regla de Rechazo** (`rejectionRule`) | Regla configurada que determina bajo qué condiciones un documento electrónico fue rechazado por la DIAN o por el Integrador Éxito (capa media FEP). Se clasifica por tipo (DIAN / Éxito) y operación | Filtro, criterio, validación |
| **Registro de Auditoría** (`auditLog`) | Registro que traza cada acción realizada sobre la configuración del sistema, identificando quién hizo el cambio, qué cambió y cuándo. Persiste en MongoDB con campos `module`, `action`, `user`, `configurationKey`, `oldValue`, `newValue` y `timestamp` | Log, historial, bitácora |
| **Configuración** (`configuration`) | Conjunto de parámetros operativos del sistema organizados por módulo, que pueden ser modificados en tiempo de ejecución. Cada cambio genera un Registro de Auditoría | Ajuste, parámetro, setting |
| **Clínica** (`clinic`) | Interfaz de gestión/administración desde la cual los operadores visualizan y gestionan los documentos electrónicos rechazados (DIAN / Éxito) para su reenvío | Panel, backoffice, consola |
| **OT Tecnológico** (`technologyOperator`) | Operador Tecnológico (proveedor externo) encargado de transmitir y validar los documentos electrónicos ante la DIAN | Proveedor, integrador DIAN, tercero |
| **Integrador Éxito** (`exitoIntegrator`) | Sistema intermediario de Éxito (FEP / capa media) que recibe los documentos desde los orígenes (POS, ERP), los transforma y los enruta hacia el OT Tecnológico. Puede rechazar documentos en este punto | FEP, middleware, capa media |
| **Rechazado** (`rejected`) | Estado de un documento electrónico que falló la validación en la DIAN o en el Integrador Éxito (FEP). Es el punto de entrada a la gestión de reenvío desde la clínica | Fallido, inválido, con error |
| **Aprobado** (`approved`) | Estado de un documento electrónico transmitido y aceptado exitosamente por la DIAN o el OT Tecnológico | Exitoso, procesado, confirmado |
| **En Progreso** (`progress`) | Estado de un documento electrónico que se encuentra en proceso activo de reenvío | Pendiente, en cola, procesando |

**Reglas:** `electronicDocument` es siempre el término genérico; usar `invoice`, `creditNote` o `debitNote` solo cuando se referencia el tipo específico. `invoice` aplica exclusivamente al flujo DIAN con estructura `RootInvoice`. Los estados `rejected`, `approved` y `progress` aplican a ambos flujos (DIAN / Éxito).
