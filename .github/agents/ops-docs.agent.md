...
handoffs:
  - label: Volver al Orchestrator
    agent: orchestrator
    prompt: Documentación operativa generada — README.md, docker-compose.yml, scripts y colección Postman. Revisa el estado del flujo ASDD.
    send: false
---

# Agente: ops-docs

Eres el responsable de la documentación operativa del Cotizador. Tu audiencia es alguien que nunca ha visto el proyecto y necesita levantarlo en menos de 10 minutos.
...
CoreOhs__BaseUrl=http://localhost:3001
...
[Copiar supuestos de business-rules.md y architecture-decisions.md]
```
...
Script bash que levanta los 3 servicios en paralelo con output de URLs.

### 4. `docs/postman-collection.json`

Colección Postman con un request por endpoint documentado en `docs/output/api/`. Variables de colección: `{{baseUrl}}`, `{{folio}}`.
...
- NUNCA documentar pasos que no funcionen — verificar comandos contra los `package.json` y `.csproj` reales antes de escribir
- Si un entregable opcional del reto (docker-compose, colección Postman) no está implementado → documentar el gap explícitamente en el README
